...
export default async function BankPage({ params, searchParams }: BankPageProps) {
  ...
  const filtered = sourceQuestions.filter((q) => {
    const hitKeyword =
      ...
      q.sourceTitle.toLowerCase().includes(keywordLower) ||
      q.province.toLowerCase().includes(keywordLower) ||
      q.questionNo.toLowerCase().includes(keywordLower);
    const hitType = typeFilter === "all" || q.primaryType === typeFilter || q.abilityTypes.includes(typeFilter);
    const hitJob = jobFilter === "all" || q.jobTags.includes(jobFilter);
    return hitKeyword && hitType && hitJob;
  });
    ...
    <main className="min-h-screen bg-slate-950 px-6 py-8 text-white md:px-10">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <form className="grid gap-3 rounded-2xl border border-white/10 bg-white/5 p-4 md:grid-cols-4">
          <input name="keyword" defaultValue={keyword} placeholder="搜索题目/来源/省份/题号" className="rounded-lg bg-slate-900 px-3 py-2" />
        ...
        <div className="space-y-3">{currentItems.map((q) => <article key={q.id} className="rounded-2xl border border-white/10 bg-white/5 p-4"><p className="text-base font-semibold">{q.questionNo}. {q.question}</p><p className="mt-2 text-sm text-slate-300">题型：{q.primaryType}</p><p className="mt-1 text-sm text-slate-300">能力：{q.abilityTypes.join("、") || "-"}</p><p className="mt-1 text-sm text-slate-300">岗位/系统：{q.jobTags.join("、") || "-"}</p><p className="mt-1 text-xs text-slate-400">来源：{q.sourceTitle || "-"} | 日期：{q.examDate || "-"} | 地区：{q.province || "-"} | 难度：{q.difficulty || "-"}</p>{q.answerStatus === "pending" ? <p className="mt-2 text-sm text-amber-300">参考答案暂未整理，可先使用 AI 模拟面试进行作答训练。</p> : null}</article>)}</div>
