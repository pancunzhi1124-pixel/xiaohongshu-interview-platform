...
export default async function HomePage() {
  ...
  const counts = Object.fromEntries(
    examTypeCategories.map((c) => [c.id, structuredCounts[c.id] && structuredCounts[c.id] > 0 ? structuredCounts[c.id] : fallbackCounts[c.id] ?? 0]),
  );
  ...
  return (
    <main id="top" className="min-h-screen overflow-x-hidden bg-slate-950 text-white">
        ...
        <div className="mx-auto grid max-w-6xl items-start gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:gap-10">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href="/interview" className="rounded-xl bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-500 px-5 py-3 font-semibold text-white">
                立即开始模拟
              </Link>
      ...
      <section id="features" className="mx-auto max-w-6xl px-6 md:px-10">
