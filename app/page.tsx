...
export default async function HomePage() {
  ...
  const fallbackCounts = interviewBanks.reduce<Record<string, number>>(
    (acc, b) => ((acc[b.id] = b.questions.length), acc),
    {},
  );
  const counts = Object.fromEntries(
    examTypeCategories.map((c) => [
      c.id,
      structuredCounts[c.id] && structuredCounts[c.id] > 0 ? structuredCounts[c.id] : (fallbackCounts[c.id] ?? 0),
    ]),
  );
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden">
      ...
      <section className="relative isolate px-6 pb-16 pt-8 md:px-10 md:pt-12">
        <div className="mx-auto max-w-6xl rounded-[2rem] border border-white/10 bg-slate-950/35">
          ...
          <div className="relative z-10 grid items-start gap-8 px-6 py-8 lg:grid-cols-[1.1fr_0.9fr] md:px-8 md:py-10">
            <div>
              <span className="inline-flex rounded-full border border-white/20 bg-white/10 px-3 py-1 text-xs text-slate-200">
                AI 面试练习平台
              </span>
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <a href="#banks" className="rounded-xl border border-white/10 bg-white/10 px-5 py-3 font-semibold text-slate-100">
                  浏览题库
                </a>
              </div>
            </div>
      ...
      <section id="usage-guide" className="mx-auto max-w-6xl px-6 md:px-10">
      ...
      <section id="banks" className="mx-auto mt-12 max-w-6xl px-6 pb-16 md:px-10">
        <h2 className="text-3xl font-bold tracking-tight text-white">五大类面试题库</h2>
